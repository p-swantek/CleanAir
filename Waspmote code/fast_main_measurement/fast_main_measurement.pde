...
// This version of the main measurement program performs its measurement loop 
...
//in fast measurement, everything turned on and configured in the setup portion
void setup() {
   ...
   //Turn on USB
    ...
    //turn on sensors that need a warmup
    ...
    delay(120000); //wait 2 min to allow android app to establish an initial connection to the waspmote's gatt server
                  //will also allow the sensors to warm up appropriately
...
void loop() {
  ...
  USB.println(); 
    ...
    //Wait 15 seconds before doing another measurement
    USB.println(F("Waiting 15 seconds..."));
